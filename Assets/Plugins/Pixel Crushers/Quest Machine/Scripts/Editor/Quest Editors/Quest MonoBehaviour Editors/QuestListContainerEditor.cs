...
namespace PixelCrushers.QuestMachine
{
    ...
    public class QuestListContainerEditor : Editor
    {
        ...
        private void OnDrawRuntimeElement(Rect rect, int index, bool isActive, bool isFocused)
        {
            ...
            var buttonWidth = (quest != null) ? 80f : 0;
            EditorGUI.BeginDisabledGroup(true);
            EditorGUI.TextField(new Rect(rect.x, rect.y + 1, rect.width - buttonWidth, EditorGUIUtility.singleLineHeight), questName);
            EditorGUI.EndDisabledGroup();
            if (quest != null)
            {
                if (GUI.Button(new Rect(rect.x + rect.width - buttonWidth, rect.y + 1, buttonWidth, EditorGUIUtility.singleLineHeight), "Inspect"))
                {
                    QuestEditorWindow.ShowWindow();
